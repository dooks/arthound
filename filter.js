...
module.exports.e926 = function(body) {
  ...
  for(var i = 0; i < new_body.length; i++) {
    ...
    new_body[i].author_link = ""; // e621 is really bad at this // x
    new_body[i].content     = new_body[i].file_url    ||  ""; // x
    new_body[i].width       = new_body[i].width       ||   0;
    new_body[i].height      = new_body[i].height      ||   0; // x
    new_body[i].favorites   = new_body[i].fav_count   ||  ""; // x
    new_body[i].preview     = new_body[i].sample_url  ||  ""; // x
    new_body[i].title       = new_body[i].id          ||   0; // x
    new_body[i].url         = new_body[i].source      ||  ""; // x
